...
function (object = object, log.price_new = log.price_new, var.soc_new = var.soc_new, log.exp_new = log.exp_new) 
{
    ...
    Result <- list(CoefCov = fit3sls$coefCov, fit3sls = fit3sls, y = y, varlist = varlist, var.soc = var.soc, shares = shares, log.price = log.price, neq = neq, y.power = y.power, nsoc = nsoc, interact = interact, 
        py.inter = py.inter, zy.inter = zy.inter, pz.inter = pz.inter, interpz = interpz, log.exp = log.exp, labels.price = labels.price, labels.soc = labels.soc, labels.share = labels.share, dim_varlist = dim_varlist, 
        fitted.w = W)
    Result
}
