...
function (object = object, file = file, sd = FALSE, limY = FALSE) 
{
    ...
    if (WDELTA) {
        nb <- dim_varlist
        MAT <- rep(1, n)
        for (i in 1:y.power) MAT <- cbind(MAT, y^i)
        for (i in 1:nsoc) MAT <- cbind(MAT, Z[, i + 1])
        if (zy.inter) {
            for (i in 1:nsoc) MAT <- cbind(MAT, y * Z[, i + 1])
        }
        for (i in 1:neq) MAT <- cbind(MAT, P[, i])
        if (py.inter) {
            for (i in 1:neq) MAT <- cbind(MAT, y * P[, i])
        }
        if (pz.inter) {
            for (i in interpz) {
                for (j in 1:neq) {
    ...
    ee = rep(1, n)
    for (j in 1:n) {
        for (i in 1:100) if (lnx[j] > quant[i]) 
            ee[j] = i
    }
    Wm = matrix(0, 100, neq)
    for (i in 1:100) {
        for (j in 1:neq) Wm[i, j] = median(W[ee == i, j])
    }
    ...
    if (WDELTA) {
        ...
        for (i in 1:100) {
            for (j in 1:neq) Wme[i, j] = median(W_ecart[ee == i, j])
        }
        for (i in 1:100) {
    ...
    limYY <- c()
    if (length(limY) < 2) {
        for (i in 1:neq) limYY <- c(limYY, c(0, summary(w[, i])[5]))
    }
    else {
        limYY <- limY
    }
    ...
    if (WDELTA) {
        points(c((1:20) * 5), Wmep[, i + 1], pch = "+", cex = 1, col = "violet")
